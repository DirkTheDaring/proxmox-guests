...
def get_hostname_from_ssh_config(name):
    """Gets hostname from ssh configuration"""
    stdout, stderr = Popen(['ssh', '-G', name],
                           stdout=PIPE).communicate()
    list = stdout.decode('utf-8').split("\n")
    for item in list:
...
class InventoryModule(BaseInventoryPlugin):
    ...
    def ping2(self, inventory, loader, sources, group):
        ...
        for host_name in group.host_names:
            ...
            if 'ansible_host' in host_vars:
                ...
                target_host = get_hostname_from_ssh_config(host_name)
    ...
    def parse_vargroups(self, inventory, loader):
    ...
    def parse_ping_filter(self, inventory, loader, sources):
        ...
        for group_name, params in groups.items():
            ...
            if enabled == True:
                ...
                result = source_group.host_names

            for host_name in result:
                self.inventory.add_child(group_name, host_name)


    def parse_groups_filter(self, inventory, loader, sources):
    ...
    def parse(self, inventory, loader, path, cache=False):
        ...
        self.parse_vargroups(inventory, loader)
        ...
        self.parse_ping_filter(inventory, loader, sources)
        self.parse_groups_filter(inventory, loader, sources)
